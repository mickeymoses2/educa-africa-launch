      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="relative w-72 bg-sidebar text-sidebar-foreground flex flex-col">
            <button onClick={() => setOpen(false)} className="absolute top-4 right-4 h-9 w-9 grid place-items-center rounded-lg bg-white/5 text-white"><X className="h-4 w-4" /></button>
    ...
    <>
      ...
      <nav className="flex-1 overflow-y-auto px-3 pb-6 space-y-0.5">
        ...
        {nav.map((item) => {
          ...
          return (
            <Link key={item.to} to={item.to} onClick={onNavigate} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition", active ? "bg-white/10 text-white" : "text-white/70 hover:text-white hover:bg-white/5")}>
              <Icon className={cn("h-[18px] w-[18px]", active ? "text-gold" : "text-white/60")} />
              <span className="flex-1 truncate">{item.label}</span>
              {item.badge && <span className="rounded-full bg-gold text-gold-foreground text-[10px] font-bold px-2 py-0.5">{item.badge}</span>}
            </Link>
          );
        })}
      </nav>
