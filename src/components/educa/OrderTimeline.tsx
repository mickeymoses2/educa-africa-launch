...
import { orderStages, type OrderStatus } from "@/data/educa";
import { cn } from "@/lib/utils";

export function OrderTimeline({ status }: { status: OrderStatus }) {
  if (status === "Cancelled") {
    return (
      <div className="rounded-2xl bg-destructive/10 text-destructive p-4 text-sm font-semibold">
        ...
        return (
          <li key={stage} className="flex gap-3">
            <div className="flex flex-col items-center">
              ...
              {i < orderStages.length - 1 && <div className={cn("w-px flex-1 my-1", done ? "bg-teal/50" : "bg-border")} />}
            </div>
            <div className="pb-4">
              <p className={cn("text-sm font-semibold", active ? "text-navy" : done ? "text-foreground" : "text-muted-foreground")}>{stage}</p>
              {active && <p className="text-xs text-muted-foreground mt-0.5">Current status — last updated today</p>}
            </div>
          </li>
        );
      })}
